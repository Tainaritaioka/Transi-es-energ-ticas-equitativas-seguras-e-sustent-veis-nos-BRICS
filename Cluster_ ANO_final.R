...
explore_year <- function(
  ...
  n_pcs <- ncol(loadings)-2
  
  colnames(loadings)[1:n_pcs] <- paste0(
    "PC",
    1:n_pcs
  )
  ...
  plot_k <- function(df_plot,y,nome){
  ...
  resultado_final <- df %>%
    mutate(
      ...
      cluster_kmedoids=pam_fit$clustering
    )
